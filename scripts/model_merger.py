...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    if 'ForTokenClassification' in config.architectures[0]:
        ...
        processor_or_tokenizer = AutoTokenizer.from_pretrained(args.hf_model_path)
    elif 'ForCausalLM' in config.architectures[0]:
        auto_model = AutoModelForCausalLM
        processor_or_tokenizer = AutoTokenizer.from_pretrained(args.hf_model_path)
    elif 'ForConditionalGeneration' in config.architectures[0]:
        auto_model = AutoModelForVision2Seq
        processor_or_tokenizer = AutoProcessor.from_pretrained(args.hf_model_path)
    else:
    ...
    print(f'Saving model and tokenizer/processor to {hf_path}')
    model.save_pretrained(hf_path, state_dict=state_dict)
    processor_or_tokenizer.save_pretrained(hf_path)
    del state_dict
    del model
