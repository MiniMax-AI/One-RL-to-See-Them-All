...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
            ...
            if optim_config is not None:
                if optim_config.lr_freeze is not None and len(optim_config.lr_freeze) > 0:
                    ...
                    for name, param in actor_module.named_parameters():
                        model_name, module_name = name.split('.')[:2]
        ...
        actor_module_fsdp = FSDP(
            ...
            use_orig_params=True,  # if optim_config.lr_freeze is not None and len(optim_config.lr_freeze) > 0 else False
        ...
        if role == 'actor' and optim_config is not None:
            ...
            for name, param in actor_module_fsdp.named_parameters():
                if param.requires_grad:
                    ...
                    if model_name == 'visual':
                        if module_name == 'blocks' or module_name == 'patch_embed':
                            lr_param = optim_config.lr_vit if optim_config.lr_vit is not None else optim_config.lr
                        elif module_name == 'merger':
                            lr_param = optim_config.lr_connector if optim_config.lr_connector is not None else optim_config.lr
                        else:
                            raise NotImplementedError(
                                f'Model name {model_name} and module name {module_name} is not supported')
                    else:
                        lr_param = optim_config.lr_llm if optim_config.lr_llm is not None else optim_config.lr
