...
from verl.single_controller.ray import RayResourcePool, RayClassWithInitArgs
...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty='kl'):
    ...
    metrics = {'rews/kl_reward': current_kl, 'training/kl_coeff': beta}
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        if self.config.data.val_files:
            ...
            val_bsz = self.config.data.get("test_batch_size", None)
            if val_bsz is not None and isinstance(val_bsz, int):
                val_bsz = int(val_bsz)
            else:
                val_bsz = len(self.val_dataset)
            val_bsz = min(val_bsz, len(self.val_dataset))
        ...
        if self.config.data.test_files:
            self.test_dataset = RLHFDataset(
                data_files=self.config.data.test_files,
                tokenizer=self.tokenizer,
                processor=self.processor,
                config=self.config.data,
            )
            ...
            test_bsz = self.config.data.get("test_batch_size", None)
            if test_bsz is not None and isinstance(test_bsz, int):
                test_bsz = int(test_bsz)
            else:
                test_bsz = len(self.test_dataset)
            test_bsz = min(test_bsz, len(self.test_dataset))
            ...
            # ) == 1, "Test dataloader must have a single batch, which inference engines will schedule the memory themselves."
        else:
    ...
    def _validate(self, dataloader: StatefulDataLoader):
        reward_tensor_lst = []
        data_source_lst = []
        ...
        for idx, test_data in enumerate(dataloader):
            ...
            reward_tensor, result_dicts = self.val_reward_fn(test_batch,
                                                             step=self.global_steps,
                                                             total_steps=self.total_training_steps,
                                                             log_sample=(idx == 0))
            sample_result_dicts.extend(result_dicts)
            # Store scores
        ...
        end_time = time.time()
        print(f"=======Benchmark {len(dataloader)} samples, takes: {round(end_time - start_time, 2)} seconds=======")
        ...
        # 生成 metric_dict
        reward_dict = {}
        if dataloader is self.val_dataloader:
            data_split = "val"
        elif dataloader is self.test_dataloader:
            data_split = "test"
        else:
            data_split = "unknown"
        ...
        for data_source, source_rewards in data_source_reward_dict.items():
            for reward_name, rewards in source_rewards.items():
                ...
                reward_dict[key] = np.mean(rewards)
    ...
    def init_workers(self):
        ...
        for resource_pool, class_dict in self.resource_pool_to_cls.items():
            ...
            print(self.ray_worker_group_cls)
            print(resource_pool)
            print(worker_dict_cls)
            print(wg_kwargs)
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            ...
            if self.test_dataloader:
                test_metrics = self._validate(self.test_dataloader)
            else:
                test_metrics = {}
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with _timer('gen_max', timing_raw):
                            ...
                            reward_baseline_tensor, result_dicts = self.reward_fn(batch,
                                                                                  step=self.global_steps,
                                                                                  total_steps=self.total_training_steps)
                            # result_dicts is a list of dicts, each dict contains: data_source, prompt, response, ground_truth, answer, final_reward, accuracy_reward, format_reward
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor, result_dicts = self.reward_fn(batch,
                                                                     step=self.global_steps,
                                                                     total_steps=self.total_training_steps)
                ...
                # save checkpoint
                if self.config.trainer.save_freq > 0 and (is_last_step
                ...
                if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and \
                    ...
                    with _timer('testing', timing_raw):
                        if self.val_dataloader:
                            val_metrics: dict = self._validate(self.val_dataloader)
                        else:
                            val_metrics = {}
                        if self.test_dataloader:
                            test_metrics: dict = self._validate(self.test_dataloader)
                        else:
                            test_metrics = {}
                        if is_last_step:
